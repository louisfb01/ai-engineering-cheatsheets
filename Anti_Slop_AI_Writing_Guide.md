...
**How to use it:** You can copy *specific sections* of this template depending on your needs, or copy the *complete template* (see Complete Template section below) and fill in all bracketed sections with your content. Paste it into ChatGPT, Claude, or another LLM.
...
```
Clear, neutral prose: professional but slightly playful and witty where it
helps understanding.
...
sample: [paste 1-2 paragraphs of your writing]
```
...
cut through the hype/noise, groundbreaking, paradigm-shifting, transformative,
pivotal, paramount, outstanding, a significant leap, delve, dive into,
embark/embarking, endeavour, realm, tapestry, vibrant, leverage, harness,
...
"recipe for disaster", "thrive", "unlock the real power".
...
Remove repeated sentence openings, banned words, and filler sentences that
...
- Write a [format: article / section / email / script / etc.] about: [topic].
- Context: [where this will be used, and why it's being written].
- Use following source material as the factual base. Do not contradict it: [paste notes, outline, quotes, links].
...
- Do NOT use these words/phrases unless I explicitly include them in my input:
  ...
  groundbreaking, paradigm-shifting, transformative, pivotal, paramount, outstanding, a significant leap,
  ...
  "the best part is", "real magic happens", "recipe for disaster", "thrive", "unlock the real power".
...
One model writes, another reviews, then revise based on feedback.

Even with this template, models won't follow every rule perfectly on the first try. Here's how to set up a revision loop.

### Step 1: Writer Model (Generate Draft)

Use the complete template above to generate version 0.

### Step 2: Editor Model (Judge the Draft)

Open a second chat. Copy-paste this prompt:
...
1. Point out where this draft breaks the rules above:
   - repetitive openings or transitions, generic intros or conclusions,
     vague or dramatic adjectives, stock email phrases or excessive politeness,
...
### Step 3: Writer Model (Revise Based on Feedback)

Go back to the writer chat. Copy-paste this prompt:

```
Here is your original draft, plus a review that points out where it broke the
style/anti-slop rules we agreed on.
...
- Add your own examples and insights as you read.
